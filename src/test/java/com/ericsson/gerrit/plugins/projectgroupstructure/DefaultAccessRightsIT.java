...
public class DefaultAccessRightsIT extends LightweightPluginDaemonTest {
  ...
  public void setUp() throws Exception {
    ...
    Files.write(tempDataDir.newFile(ProjectConfig.PROJECT_CONFIG).toPath(), defaultAccessRights.getBytes());
    super.setUp();
  ...
  public void shouldConfigureForRootProject() throws Exception {
    ...
    assertThat(refsSection.getPermission(Permission.OWNER).getRules().get(0).getGroup().getUUID()).isEqualTo(ownerUUID);
    assertThat(refsSection.getPermission(Permission.READ).getRules().get(0).getGroup().getUUID()).isEqualTo(ownerUUID);
    assertThat(refsSection.getPermission(Permission.READ).getExclusiveGroup()).isTrue();
    assertThat(refsSection.getPermission(Permission.PUSH).getRules().get(0).getGroup().getName()).isEqualTo("Administrators");
    ...
    assertThat(refsHeadsSection.getPermission(Permission.CREATE).getRules().get(0).getGroup().getUUID()).isEqualTo(ownerUUID);
    assertThat(refsHeadsSection.getPermission(Permission.PUSH).getRules().get(0).getGroup().getUUID()).isEqualTo(ownerUUID);
    assertThat(refsHeadsSection.getPermission(Permission.LABEL + "Code-Review").getRules().get(0).getGroup().getUUID()).isEqualTo(ownerUUID);
    assertThat(refsHeadsSection.getPermission(Permission.SUBMIT).getRules().get(0).getGroup().getUUID()).isEqualTo(ownerUUID);
  }
  ...
  public void shoudNotConfigureForNonRootProject() throws Exception {
    ...
    assertThat(projectCache.get(new Project.NameKey(projectName)).getConfig().getAccessSections().size()).isEqualTo(0);
  }
}
