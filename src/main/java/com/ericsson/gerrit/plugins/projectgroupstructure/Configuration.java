...
class Configuration {
  ...
  Configuration(PluginConfigFactory pluginConfigFactory, @PluginName String pluginName) {
    ...
    regexNameFilter = config.getString(pluginName, null, NAME_REGEX);
    if (regexNameFilter == null || isInvalidRegex(regexNameFilter)) {
      regexNameFilter = DEFAULT_NAME_REGEX_VALUE;
      if (isInvalidRegex(regexNameFilter)) {
  ...
  private boolean isInvalidRegex(String regex) {
    return !"/".matches(regexNameFilter) || " ".matches(regexNameFilter);
  }
}
