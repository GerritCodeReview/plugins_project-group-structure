 ...
 * <p>
 * Default access rights are read from <review_site>/data/project-group-structure/project.config.
 * The format of that file is the same as regular project.config except that
 * group, in addition to be a group name, can be set to token ${owner} instead
 * which will be replaced by the group owning the project.
 */
@Singleton
public class DefaultAccessRights implements NewProjectCreatedListener {
  ...
  public DefaultAccessRights(
      MetaDataUpdate.User metaDataUpdateFactory, ProjectCache projectCache, GroupCache groupCache, @PluginData Path dataDir) {
    this.groupCache = groupCache;
    this.projectCache = projectCache;
    this.metaDataUpdateFactory = metaDataUpdateFactory;
    defaultAccessRightsConfig = new FileBasedConfig(
        dataDir.resolve(ProjectConfig.PROJECT_CONFIG).toFile(), FS.DETECTED);
    try {
      ...
      log.error(
          ...
          defaultAccessRightsConfig.getFile().getAbsolutePath(), e.getMessage(),
          e);
    }
  }
  ...
  public void onNewProjectCreated(NewProjectCreatedListener.Event event) {
    ...
    if (projectName.contains("/")
        || defaultAccessRightsConfig.getSubsections(ProjectConfig.ACCESS).isEmpty()) {
      return;
    }
    ...
    try (MetaDataUpdate md = metaDataUpdateFactory.create(project.getProject().getNameKey())) {
      ...
      for (String refName : defaultAccessRightsConfig.getSubsections(ProjectConfig.ACCESS)) {
        if (RefConfigSection.isValid(refName) && isValidRegex(refName)) {
          ...
          for (String varName : defaultAccessRightsConfig.getStringList(ProjectConfig.ACCESS, refName,
          ...
          for (String varName : defaultAccessRightsConfig.getNames(ProjectConfig.ACCESS, refName)) {
            if (Permission.isPermission(varName)) {
              Permission perm = as.getPermission(varName, true);
              for (String ruleString : defaultAccessRightsConfig.getStringList(ProjectConfig.ACCESS, refName, varName)) {
                PermissionRule rule;
                try {
                  rule = PermissionRule.fromString(ruleString.replaceAll(Pattern.quote(OWNER_TOKEN), projectName + "-admins"), Permission.hasRange(varName));

                } catch (IllegalArgumentException notRule) {
                  log.error("Invalid rule in "
              ...
              log.error("Invalid permission {}", varName);
            }
          }
        }
      }
